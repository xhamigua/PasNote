 ...
 * Ported by CodeCoolie@CNSW 2008/04/02 -> $Date:: 2013-11-18 #$
 *)
...
function mid_pred(a, b, c: Integer): Integer; {$IFDEF USE_INLINE}inline;{$ENDIF}
...
function av_clip_uintp2_c(a, p: Integer): Cardinal; {$IFDEF USE_INLINE}inline;{$ENDIF}
function av_sat_add32_c(a, b: Integer): Integer; {$IFDEF USE_INLINE}inline;{$ENDIF}
function av_sat_dadd32_c(a, b: Integer): Integer; {$IFDEF USE_INLINE}inline;{$ENDIF}
function av_clipf(a, amin, amax: Single): Single; {$IFDEF USE_INLINE}inline;{$ENDIF}
(* math *)
function ff_get_fourcc(const s: PAnsiChar): Integer; {$IFDEF USE_INLINE}inline;{$ENDIF}
...
(* median of 3 *)
function mid_pred(a, b, c: Integer): Integer;
begin
  if a > b then
  begin
    if c > b then
 ...
 * @return clipped value
 *)
function av_clip_uintp2_c(a, p: Integer): Cardinal;
begin
 ...
 *)
function av_sat_add32_c(a, b: Integer): Integer;
begin
 ...
 * @return sum with signed saturation
 *)
function av_sat_dadd32_c(a, b: Integer): Integer;
begin
 ...
 *)
function ff_get_fourcc(const s: PAnsiChar): Integer;
begin
  Result := PByte(s)^ + PByte(s + 1)^ shl 8 + PByte(s + 2)^ shl 16 + PByte(s + 3)^ shl 24;
end;
